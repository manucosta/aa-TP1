...
outfile = open('mask.txt', 'w')
filtro = select.get_support(indices=True)
for n in filtro:
  outfile.write(str(n))
  outfile.write('\n')
outfile.close()
...
fout = open('kbest-dectree.pickle','w')
pickle.dump(best_clf,fout)
fout.close()

# Random Forest
print "Random Forest"
clf = ensemble.RandomForestClassifier(max_features = 0.5, max_leaf_nodes = None, min_samples_split = 4, criterion = 'gini', n_estimators = 20, n_jobs=4)
...
fout = open('kbest-ranfor.pickle','w')
...
kf = KFold(72000, n_folds=3, shuffle=True)
best_score = 0
best_clf = 0
for train_index, test_index in kf:
  print("TRAIN:", train_index, "TEST:", test_index)
...
fout = open('kbest-gaussianNB.pickle','w')
pickle.dump(clf,fout)
fout.close()
